...
### Ukrainian: numbers five and up now put the noun in the right case
Ukrainian sentences with a numeral used to leave the noun in its dictionary form, so «Він читає сім книги» / «Ми бачимо шість аеропорти» shipped as correct. Ukrainian requires the noun to change form after five and up — «сім книг», «шість аеропортів». The engine now knows this the same way it already knows it for Polish. Six core nouns (book, phone, passport, airport, hotel, pan) carry the required form and read correctly with any number; the rest have not been authored yet and the number is skipped for those pairings rather than shipping the wrong grammar. More nouns land as they are authored.
